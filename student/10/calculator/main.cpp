...
bool check_if_numbers(std::vector<string> pieces){
    bool return_value = true;

    for(auto i : pieces ){
        double asd;
        if(! string_to_double(i, asd)){
            return_value = false;
            break;
        }
    }
...
int main() {
    ...
    while ( true ) {
        ...
        string command_to_be_executed_upper = "";

        for(char i : command_to_be_executed){
            command_to_be_executed_upper += toupper(i);
        }

        pieces.erase(pieces.begin(), pieces.begin() + 1);
        vector<string>::size_type input_length = pieces.size();
        ...
        for(auto command : COMMANDS ){
            if( command.str == command_to_be_executed_upper ){
                has_found = true;
                if( command.parameter_number == input_length ){
                    if(! command.exit ){
                        if( check_if_numbers(pieces) ){
                            cout << command.action(stod(pieces.at(0)), stod(pieces.at(1))) << endl;
                        } else {
