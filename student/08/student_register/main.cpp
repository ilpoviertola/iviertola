...
void change_phone_number(std::string student_nmbr, std::string file_name,
                         std::map< std::string, Student* >& alphabetical_order,
                         std::map< std::string, Student* >& numerical_order) {

    if( numerical_order.find(student_nmbr) != numerical_order.end()){
        std::string new_phone_nmbr = "";
        std::cout << "Enter a new phone number: ";
        getline( std::cin, new_phone_nmbr );
        std::cout << "\n";

        if( is_valid_phone_number( new_phone_nmbr )){
            numerical_order.at(student_nmbr)->phone_number = new_phone_nmbr;
            alphabetical_order.at(numerical_order.at(student_nmbr)->user_id)->phone_number = new_phone_nmbr;

            std::ofstream fileObj;
            std::vector<std::string> tmp;

            fileObj.open(file_name, std::ofstream::out | std::ofstream::trunc);


            for(std::map<std::string, Student*>::iterator itMap = alphabetical_order.begin();
                itMap != alphabetical_order.end(); ++itMap ){
                std::ostringstream vts;
                tmp.push_back(itMap->second->student_number);
                tmp.push_back(itMap->second->user_id);
                tmp.push_back(itMap->second->name);
                tmp.push_back(itMap->second->phone_number);
                tmp.push_back(itMap->second->email);
                tmp.push_back(itMap->second->skype);
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            if(parts.size() != 2){
                ...
                change_phone_number( parts.at(1), file_name, user_ids, student_numbers );
            }
